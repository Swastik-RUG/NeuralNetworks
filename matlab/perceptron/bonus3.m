...
if isfile(learning_rate_by_epochs_state_store) && USE_STATE_STORE == 1
    ...
    for e=1:size(learning_rate_by_epochs,1)
        ...
        title(sprintf('Learning Rate Analysis for Epochs = %d', epoch));
        xlabel('Alpha = P/N');
        ylabel('Success Rate');
    ...
    for ep=1:length(epochs)
        ...
        title(sprintf('Learning Rate Analysis for Epochs = %d', epochs(ep)));
        xlabel('Alpha = P/N');
        ylabel('Success Rate');
...
if isfile(learning_rate_by_dimensions_state_store) && USE_STATE_STORE == 1
    ...
    for e=1:size(learning_rate_by_dimensions,1)
        ...
        xlabel('Alpha = P/N');
        ylabel('Success Rate');
        legend(cellstr(num2str(N', 'Dimensions=%-d')))
    end
else
    for d=1:length(N)
        subplot(2,2,d)
        ...
        for i=1:length(c_values)
            ...
            for j=1:length(alphas)
                alpha = alphas(j);
                success_for_diff_c(i, j) = run_perceptron(alpha, dim, epoch, itr, c);
            end
        end
